...
class SnakeAgent:
    ...
    def helper_func(self, state):
        ...
        if [head_x, head_y - 1] not in body_list:
            available_moves += 1
        
        if [head_x, head_y + 1] not in body_list:
            available_moves += 1
        
        if [head_x - 1, head_y] not in body_list:
            available_moves += 1
        ...
        conditions = [wall_hit[0], wall_hit[1], direction_of_food[0], direction_of_food[1], snake_body[0], snake_body[1], snake_body[2], snake_body[3], available_moves]
    ...
    def agent_action(self, state, points, dead):
        ...
        def change(state,dead,points,previous_state,previous_action):
            ...
            change = q + alpha * (rew + self.gamma * action - q)
